...
struct SmartMapsViewCustomized: UIViewRepresentable {
    ...
    @Binding var displayed_host : HostAboutMe?
    private let zoom: Float = 5
    ...
    func updateUIView(_ mapView: GMSMapView, context: Context) {
        ...
        @State var charger_markers: [GMSMarker] = topchargerarray.map {
            let marker = GMSMarker(position: CLLocationCoordinate2D(latitude: Double($0.latitude) ?? 42.2808 , longitude: Double($0.longitude) ?? -83.7430 ) )
    ...
    class Coordinator: NSObject, GMSMapViewDelegate {
       let owner: SmartMapsViewCustomized       // access to owner view members,
        @State private var data: [String : Any] = [String:Any]()

       init(owner: SmartMapsViewCustomized) {
         self.owner = owner
       }
